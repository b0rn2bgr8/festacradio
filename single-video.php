...
<section id="page-cover">
    ...
    <section class="col-md-6 col-md-offset-1">
     ...
     <?php
        ...
        $query_video = "select * from video where id = '$id'";
        $run_query = mysqli_query($con, $query_video);
        
        while($row_video = mysqli_fetch_array($run_query)){
            
            $name = $row_video['title'];
            $path = $row_video['path'];
            $a_id = $row_video['artist'];
            ...
            while($row_aquery = mysqli_fetch_array($run_aquery)){
                ...
                echo "<section class='panel-heading'>$name</section>
                   ...
                   <video src='media/video/$path' height='400' width='450' controls></video>
